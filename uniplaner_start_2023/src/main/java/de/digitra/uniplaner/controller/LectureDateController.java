...
public class LectureDateController implements ILectureDateController {
    ...
    public ResponseEntity<LectureDate> createLectureDate(@RequestBody LectureDate lecturedate) throws BadRequestException {
        log.info("REST request to save LectureDate : {}", lecturedate);
    ...
    @Override
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Bei einer erfolgreichen Ausführung wird eine ResponseEntity mit Status Code {@code 200 (OK)} und im Body die aktualisierte Instanz von LectureDate zurückgeliefert."),
        @ApiResponse(responseCode = "400", description = "Status Code {@code 400 (Bad Request)} wird zurückgeliefert, falls der übergebene Parameter lecturedate nicht zulässig ist. Dies ist der Fall, falls er eine Id mit dem Wert null hat."),
     ...
     *
     * @return Eine {@link ResponseEntity} mit Status Code {@code 200 (OK)} und im Body eine Liste von Ressourcen vom Typ LectureDate.
     */
    @Override
    @GetMapping
    public ResponseEntity<List<LectureDate>> getAlllecturedates() {
        log.debug("REST request to get all lecturedates");
        List<LectureDate> lectureDates = lecturedateService.findAll();
        List<LectureDate> filteredLectureDates = lectureDates.stream()
            .filter(date -> date.getId() != 0 && date.getStartDate() != null)
            .collect(Collectors.toList());
        return ResponseEntity.ok(filteredLectureDates);
    }
    ...
    @Override
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Bei einer erfolgreichen Ausführung wird Eine {@link ResponseEntity} mit Status Code {@code 200 (OK)} und im Body die gesuchte Ressource vom Typ LectureDate zurückgeliefert."),
        @ApiResponse(responseCode = "404", description = "Status Code {@code 404 (Not Found)} wird zurückgeliefert, falls die Ressource mit der angegebenen Id nicht gefunden werden konnte.")
    })
    @GetMapping("/{id}")
